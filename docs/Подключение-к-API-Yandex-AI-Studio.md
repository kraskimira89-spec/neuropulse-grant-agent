...
Не передавайте ключ посторонним и не публикуйте в репозитории. Храните его в `config/config.json` (файл в `.gitignore` не коммитьте) или в переменных окружения (`.env`).
...
Не коммитьте `config.json` с ключом в Git. Либо храните ключ только в `.env` (см. ниже).

### Вариант Б: переменные окружения (.env)
